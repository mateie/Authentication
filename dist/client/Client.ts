...
import { RsoAuthFlow } from '../service/AuthFlow';

import { RsoUserAuth, type RsoAuth, type RsoAuthExtend } from "../service/User";
import { RsoMultifactorAuth } from "../service/Multifactor";
...
declare interface RsoClient {
    emit<EventName extends keyof RsoAuthClientEvent>(name: EventName, ...args: Parameters<RsoAuthClientEvent[EventName]>): any;
    on<EventName extends keyof RsoAuthClientEvent>(name: EventName, callback: RsoAuthClientEvent[EventName]): any;
    once<EventName extends keyof RsoAuthClientEvent>(name: EventName, callback: RsoAuthClientEvent[EventName]): any;
    off<EventName extends keyof RsoAuthClientEvent>(name: EventName, callback?: RsoAuthClientEvent[EventName]): any;
}
...
class RsoClient extends EventEmitter {
    ...
    public constructor(config: RsoAuthConfig = {}) {
        ...
        const _normalAxiosConfig: AxiosRequestConfig = {
            ...
            httpsAgent: new HttpsAgent({ keepAlive: true, ciphers: ciphers.join(':'), honorCipherOrder: true, minVersion: 'TLSv1.2', maxVersion: 'TLSv1.3' }),
     ...
     * @returns {void}
     */
    public fromJSON(data: RsoAuthSave): void {
        this.cookie = {
    ...
    public async login(username: string | Function, password: string | Function): Promise<void> {
        ...
        const _extraData: RsoAuthExtend = {
            client: {
        ...
        const NewAuth: RsoAuth = await RsoUserAuth.login(this.toJSON(), username, password, _extraData);
    ...
    public async verify(verificationCode: number | string | Function): Promise<void> {
        ...
        const NewAuth: RsoAuth = await RsoMultifactorAuth.verify(this.toJSON(), Number(verificationCode), _extraData);
